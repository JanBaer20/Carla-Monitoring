...
class VehicleDataTest(Criterion):
    """
    ...
    def __init__(self, actor, world: World, debug_mode, name="VehicleDataTest", terminate_on_failure=False):
        super(VehicleDataTest, self).__init__(name, actor, 0,
    ...
    def update(self):
        ...
        print(self._actor.get_light_state() & carla.VehicleLightState.RightBlinker)
        print(self._contact_lanes)
        if self._current_lane in self._contact_lanes:
            if self._contact_lanes[self._current_lane] == DataContactLocation.LEFT:
                print("Right Lane Change")
                if self._actor.get_light_state() & carla.VehicleLightState.RightBlinker == 0:
                    self.send_violation(b"Lane change without indicator")
            elif self._contact_lanes[self._current_lane] == DataContactLocation.RIGHT:
                print("Left Lane Change")
                if self._actor.get_light_state() & carla.VehicleLightState.LeftBlinker == 0:
                    self.send_violation(b"Lane change without indicator")

        self._contact_lanes.clear()
        if lane_id > 0:
            ...
            for lane in self._map_helper.get_contact_lane_infos(self._map_helper.get_lane_for_lane_id(lane_id)):
                #print(lane)
                if lane.contact_type == DataContactType.LANE_CHANGE:
                    self._contact_lanes[lane.ad_map_lane_id] = lane.contact_location
            print("Road")
...
def monitor(args):
    ...
    test = VehicleDataTest(actor, world=world, debug_mode=True,
